...
function mergeEnrollmentStudentRecord(existing: StudentRecord | undefined, student: EnrollmentStudentRecord): StudentRecord {
  ...
  return {
    ...existing,
    ...student,
    // assignedOfferingIds/assignedOfferingDeadlines/status are exclusively owned by
    // setStudentOfferingAssignment now (course assignment) and the course-completion logic
    // elsewhere — no caller that reaches this generic merge (updateStudent, bulk upload, group
    // edits, HR sync, patchManagerState's roster patch) ever *intends* to change them.
    // EnrollmentStudentInput explicitly omits all three for exactly this reason. They only ride
    // along in `student` because the caller's own local snapshot still carries whatever these
    // fields happened to be — which, for assignedOfferingIds/assignedOfferingDeadlines
    // especially, is routinely stale within seconds of an assignment, since they change far more
    // often than any other roster field. Always keeping the freshly-read `existing` value instead
    // of trusting the patch is what actually closes the "course (or its deadline) vanishes
    // sometime after being assigned" bug: any unrelated edit to this student (or a completely
    // different manager's stale roster save touching a batch that happens to include them) can no
    // longer silently revert it.
    assignedOfferingIds: existing.assignedOfferingIds,
    assignedOfferingDeadlines: existing.assignedOfferingDeadlines,
